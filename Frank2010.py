...
from src.constants import LACOS
import re
import src.RnR as RnR
import math
import scipy.stats.stats as stats
import src.data.Frank2011.Frank2010Results as frr
...
class experiment:
    ...
    def mse(self, modelData):
        values_model = [v for (_,v) in sorted(modelData.items())]
        values_humans = [v for (_,v) in sorted(self.expResults.avg_performance.items())]
        values_humans = map(lambda x: x*100, values_humans)
        mse=0.0
        for i in range(0,len(values_humans)):
            mse += (values_humans[i] - values_model[i])**2 
        mse = mse / len(values_humans)
...
class exp_cond:
    ...
    def __init__(self, expId, condition, shuffle=False):
        ...
        self.SYLLABLES = ("ba", "bi", "da", "du", "ti", "tu", "ka", "ki", "la", "lu", "gi", "gu", "pa", "pi", "va", "vu", "zi", "zu")
        self.expId = expId
    ...
    def generateTest(self):
        ...
        for _ in range(0,self.TEST_LENGTH):
            r = randGlobal.myRandom.randint(0,len(self.words)-1)
            w = self.words[r]
            l = len(w)
            pw = self.__getRandomDistractor(l)
            pair=[w,pw]
            if (self.shuffle):
                randGlobal.myRandom.shuffle(pair)
            test.append(pair)
        if (self.shuffle):
            self.test=randGlobal.myRandom.shuffle(test)
        vprint(test)
        return test
    ...
    def __getRandomDistractor(self,length):
        ...
        while len(distractor) != length:
            r = randGlobal.myRandom.randint(0,len(self.distractors)-1)
            distractor = self.distractors[r]
        return distractor

    '''Returns next randomly chosen word'''
    def __choose_next(self,block):
        r = randGlobal.myRandom.randrange(0,len(block))
        return block[r]
    ...
    def __randomizedStimuli(self):
        ...
        while len(iterationnext) > 0:       
            ...
            if actsl % self.sentence_length == 0 or self.satisfies_constraints(finalStimuli,next): #there are no constraints at the beginning of the sentence
    ...
    def __randomVocabulary(self):
        ...
        if self.types % 3 == 0:
            ...
            r = randGlobal.myRandom.randint(0,2)
        ...
        for l in lengths:
            word=""
            for i in range (0,l):
                r = randGlobal.myRandom.randrange(0,len(self.SYLLABLES))
