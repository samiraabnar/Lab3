...
import sys
sys.path.append("/home/rgalhama/Research/SegmentsRules/Segmentation/Modelling/RnR/RnR")
import re
import time
import randGlobal
import pickle
...
class RnR:
    ...
    def __init__(self, A, B, C, D, munp, muwp, nmax, mcapacity="variable"):
        ...
        self.ltypes = dict(zip(range(1, self.nmax + 1), [0] * self.nmax)) #dict of length-ntypes
    ...
    def twoAFC(self, x, y):
        ...
        l_x = len(x) / 2
        l_y = len(y) / 2
        if x in self.subj_freq[l_x].keys():
            sf_x = self.subj_freq[l_x][x]
        if y in self.subj_freq[l_y].keys():
        ...
        die = randGlobal.myRandom.uniform(0.0, 1.0)
    ...
    def treatSequence(self, n, sequence, prevSyllable):
        ...
        r1 = randGlobal.myRandom.uniform(0.0, 1.0)
        r2 = randGlobal.myRandom.uniform(0.0, 1.0)

        if  r1 < self.recognitionProbability(n, sequence):
            if verbose:
                vprint (sequence + " recognized!")
            if sequence != self.PAUSE and sequence not in self.subj_freq[n].keys():
                self.ltypes[n] += 1
                self.ntypes+=1
...
''' Version with pause represented with exponential parameter mu.
    munp is used if there was no pause before the segment that is being processed
    muwp is used if there was a pause before the segment that is being processed
'''
class RnRv3(RnR):
    
    def __init__(self, A, B, D, munp, muwp, nmax, mcapacity="variable"):
        assert(0<=A<=1 and 0<=B<=1 and 0<=D<=1 and nmax>0)
        C = 1
        RnR.__init__(self, A, B, C, D, munp, muwp, nmax, mcapacity)
...
if __name__ == '__main__':
    ...
    mm = RnRv3(A=0.5, B=0.6, D=0.85, munp=0.5, muwp=0.7, nmax=5)
    candidates = mm.createCandidates(exp.stream)
    print candidates

    # Print subj_freq of sequences that have been memorized, along with the type of sequence (word or part word)
    # for model in range(1, nmax + 1):
    #     print "Model: ", model
    #     for c in mm.subj_freq[model]:
    #         typesequence = "word" if c in words else "partword"
    #         print c, mm.subj_freq[model][c], typesequence

#    print "////2AFC/////"
#    test=generateTestPena("EXP1",stream)
#    print test
#    for pair in test:
#        chosen = mm.twoAFC(pair[0],pair[1])	
#        print "Chosen: ", chosen
